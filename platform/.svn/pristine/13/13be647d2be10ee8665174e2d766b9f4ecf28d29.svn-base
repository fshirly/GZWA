...
dutymanager.editduty = {
		basePath : getRootName(),
		oldOrderId : [],
		update : function () {		
			var that = this;
			if (checkInfo ('#edit_duty_form') && this.validateDuty()) {
				var orders = this.getExistOrder(true);
				if (orders.length === 0) {
					f.messager.alert('提示', '值班计划必须包含值班班次,请检查！' , 'info');
					return;
				}
				orders = f.map(orders, function (order) {if(!order.orderIdOfDuty){return order;}});
				f.ajax({
					url : that.basePath + '/platform/dutymanager/duty/update',
					data : f('#edit_duty_form').serialize() + '&' + f.param({'orders' : orders}) + '&orderIds=' + that.oldOrderId.join(',') +'&level='+$('#cc').combobox('getValue'),
					success : function (data) {
						if (data) {
							window.frames['component_2'].dutymanager.duty.reload();
							f('#popWin').window('close');
						} else {
							f.messager.alert('提示', '编辑值班信息失败，请检查！' , 'info');
						}
					}
				});
			}
		...
		addOrder : function () {/*跳转到添加值班班次页面*/
			...
			f('#duty_orders').combobox('loadData', that.getIdleOrder(that.getExistOrder().join(',')));/*重新加载班次信息*/
		...
		delGridData : function (index, orderIdOfDuty) {/*删除值班班次*/
			var that = this, $dutyGrid = f('#orders');
			f.messager.confirm('提示', '确定删除该班次信息?', function(r){
				if (r) {
					if (orderIdOfDuty) {
						that.oldOrderId.push(orderIdOfDuty);
						/*f.ajax({
							url : that.basePath + '/platform/dutymanager/duty/deleteOrder',
							data : {orderIdOfDuty : orderIdOfDuty},
							success : function (data) {
								if (data) {
									f('#orders').datagrid('deleteRow', index);
								} else {
									f.messager.alert('提示', '删除值班记录下的班次信息失败！', 'info');
								}
							}
						});*/
					} 
					$dutyGrid.datagrid('deleteRow', index);
					var datas = $dutyGrid.datagrid('getData');
					$dutyGrid.datagrid('loadData', datas.rows);
				}
			});
		},
		validateDuty : function () {/*验证值班日期*/
			...
			f.ajax({
				...
				data : {dutyDate : dutyDate, dutyId : f('[name="id"]').val()},
				async : false,
				success : function (data) {
					isExist = data;
				}
...
f(function(){
	f('#leaderId').createSelect2({
		...
		name : 'name',// 显示名称
		id : 'id',
		initVal : {leaderId : f('#leaderId').attr('value')}
	});
	$("#cc").combobox({
	...
	$("#cc").combobox('select', $('#duty_level').val());
	(function(){
		f('#orders').datagrid({
			...
			columns : [ [
			             ...
			             },
			             {
			            	 ...
			            	 width : 80,
			            	 formatter : function(value,row,index){
			            		 return '<a href="javascript:dutymanager.editduty.delGridData(' + index + ',' + row.orderIdOfDuty + ')">删除</a>';
			            	 }
			             }] ]
		});
	})();
